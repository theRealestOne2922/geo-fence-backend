...
print(f"[Setup] Geo-fence radius: {GEO_FENCE_RADIUS_METERS}m. Ready.")
...
def encrypt():
    ...
    print(f"[Encrypt] Locked @ ({lat:.6f}, {lon:.6f}) GeoHash={ghash} ID={file_id}")
...
def decrypt():
    ...
    print(f"[Decrypt] From ({lat:.6f}, {lon:.6f}) dist={distance:.1f}m limit={GEO_FENCE_RADIUS_METERS}m")
